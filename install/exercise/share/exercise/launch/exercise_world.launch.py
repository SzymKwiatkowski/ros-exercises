...
def generate_launch_description():
    pkg_exercise = get_package_share_directory(
        'exercise')
    
    gui_config_path = os.path.join(pkg_exercise, 'config/exercise.config')
    ...
    ignition_gazebo = IncludeLaunchDescription(
        ...
        launch_arguments=[
            ('gz_args', ['--gui-config ' + gui_config_path, ' exercise.sdf'])
        ]
    )
